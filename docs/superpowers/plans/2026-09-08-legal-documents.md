...
Run:
```bash
cd /Users/pablohpsilva/Documents/camscanner-light && pnpm --filter @camscanner/legal-content generate
...
In `apps/web/README.md`, add `terms.html`, `faq.html`, and `legal/` to the Pages list, and add a line stating these are **generated** by `pnpm --filter @camscanner/legal-content generate` and must not be hand-edited.
...
**Interfaces:**
- Consumes: `pnpm --filter @camscanner/legal-content generate`.
...
if ! git diff --quiet -- "${OUTPUTS[@]}"; then
  ...
  echo "Fix: pnpm --filter @camscanner/legal-content generate, then commit the result."
  echo
