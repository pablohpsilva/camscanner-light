...
void main() {
  ...
  testWidgets('scanned text becomes selectable text in the exported PDF',
    ...
    // ignore: avoid_print
    print('E2E ocrText="${pages.single.ocrText}" words=${pages.single.ocrWords.length}');
    ...
    final pdfBytes = await const PdfBuilder(textLayer: OcrPdfTextLayer())
        .build(pages, compress: false);
    final pdfText = _dec(pdfBytes).toUpperCase();
    expect(pdfText.contains('HELLO'), isTrue,
        reason: 'PDF must carry the recognized text as an invisible selectable layer');
