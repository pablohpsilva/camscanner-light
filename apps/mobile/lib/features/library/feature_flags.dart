...
///
/// Every flag defaults ON except [fax] (no fax provider is wired yet) and
/// [shareLink] (C4a — no link-share channel is wired yet, so it is hidden by
/// default; re-enable with `--dart-define=FEATURE_SHARE_LINK=true`). A disabled
/// flag HIDES its control entirely — see PageViewerScreen
...
class FeatureFlags {
  ...
  const FeatureFlags({
    ...
    this.rotate = const bool.fromEnvironment(
      'FEATURE_ROTATE',
      defaultValue: true,
    ),
    this.filter = const bool.fromEnvironment(
      'FEATURE_FILTER',
      defaultValue: true,
    ),
    this.viewText = const bool.fromEnvironment(
      'FEATURE_VIEW_TEXT',
      defaultValue: true,
    ),
    this.retake = const bool.fromEnvironment(
      'FEATURE_RETAKE',
      defaultValue: true,
    ),
    this.share = const bool.fromEnvironment(
      'FEATURE_SHARE',
      defaultValue: true,
    ),
    this.deletePage = const bool.fromEnvironment(
      'FEATURE_DELETE_PAGE',
      defaultValue: true,
    ),
    this.rename = const bool.fromEnvironment(
      'FEATURE_RENAME',
      defaultValue: true,
    ),
    this.merge = const bool.fromEnvironment(
      'FEATURE_MERGE',
      defaultValue: true,
    ),
    this.split = const bool.fromEnvironment(
      'FEATURE_SPLIT',
      defaultValue: true,
    ),
    ...
    this.exportPdf = const bool.fromEnvironment(
      'FEATURE_EXPORT_PDF',
      defaultValue: true,
    ),
    this.shareImage = const bool.fromEnvironment(
      'FEATURE_SHARE_IMAGE',
      defaultValue: true,
    ),
    ...
    this.print = const bool.fromEnvironment(
      'FEATURE_PRINT',
      defaultValue: true,
    ),
    ...
    this.shareLink = const bool.fromEnvironment(
      'FEATURE_SHARE_LINK',
      defaultValue: false,
    ),
    this.fax = const bool.fromEnvironment('FEATURE_FAX', defaultValue: false),
    this.idCard = const bool.fromEnvironment(
      'FEATURE_ID_CARD',
      defaultValue: true,
    ),
    this.scan = const bool.fromEnvironment('FEATURE_SCAN', defaultValue: true),
    this.import = const bool.fromEnvironment(
      'FEATURE_IMPORT',
      defaultValue: true,
    ),
  });
