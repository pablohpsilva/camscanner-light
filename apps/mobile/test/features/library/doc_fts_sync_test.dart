...
void main() {
  ...
  Future<int> insertDoc(String name) => db
      .into(db.documents)
      .insert(
  ...
  Future<int> insertPage(int docId, int pos, {String? ocr}) => db
      .into(db.pages)
      .insert(
  ...
  test(
    'multi-word AND matches across DIFFERENT pages of one document',
    () async {
  ...
  test('trigger fills the index when ocr_text is set via UPDATE', () async {
    ...
    await (db.update(db.pages)..where((t) => t.id.equals(pageId))).write(
      const PagesCompanion(ocrText: Value('a KEYWORD here')),
    );
    expect(await matchDocs('"keyword"'), [id]);
  });

  test(
    'clearing ocr_text and deleting pages leaves no orphan doc_fts row',
    () async {
      final id = await insertDoc('Doc');
      ...
      await (db.update(db.pages)..where((t) => t.id.equals(p1))).write(
        const PagesCompanion(ocrText: Value(null)),
      );
      expect(await matchDocs('"keyword"'), [id]);
