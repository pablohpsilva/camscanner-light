...
void main() {
  ...
  test(
    'multi-page: one PDF page per input page, in order, at each image aspect',
    () async {
      ...
      final f1 = File('${tmp.path}/p1.jpg')
        ..writeAsBytesSync(makeJpeg(120, 60));
      final f2 = File('${tmp.path}/p2.jpg')
        ..writeAsBytesSync(makeJpeg(80, 160));
      final f3 = File('${tmp.path}/p3.jpg')
        ..writeAsBytesSync(makeJpeg(200, 100));
      final pages = [
      ...
      final widths = RegExp(
        r'/Width\s+(\d+)',
      ).allMatches(s).map((m) => int.parse(m.group(1)!)).toList();
      final heights = RegExp(
        r'/Height\s+(\d+)',
      ).allMatches(s).map((m) => int.parse(m.group(1)!)).toList();
      ...
      expect(heights, [
        ...
        160,
        100,
      ], reason: 'heights follow page order (aspect)');
  ...
  test(
    'auto-orients: EXIF-6 200x100 fixture -> oriented page 100x200',
    () async {
      final pdf = await const PdfBuilder().build([page()]);
      final m = RegExp(
  ...
  test(
    'metadata-clean: no personal/device info (author/producer/creator/date)',
    () async {
  ...
  test(
    'seam: overlayFor invoked per page; text injected; none when image-only',
    () async {
      ...
      final pdf = await PdfBuilder(
        textLayer: spy,
      ).build([page()], compress: false);
      expect(
        spy.calls.single.position,
        1,
        reason: 'overlayFor called with the page',
      );
      expect(
        dec(pdf).contains('SEAMTEXT'),
        isTrue,
        reason: 'injected text present',
      );

      final imageOnly = await const PdfBuilder().build([
        page(),
      ], compress: false);
  ...
  test(
    'E2: uses displayPath — reads flat file when flatImagePath is set',
    () async {
