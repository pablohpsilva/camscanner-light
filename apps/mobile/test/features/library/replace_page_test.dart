...
void main() {
  ...
  test(
    'replacePage (full-frame) overwrites the image and clears corners',
    () async {
      final r = repo();
      final doc = await r.createFromCapture(
        freshCapture('c1.jpg'),
        corners: corners,
      ); // page 1 starts with a flat + corners
  ...
  test(
    'replacePage on a non-existent position throws DocumentSaveException',
    () async {
      final r = repo();
  ...
  test(
    'replacePage reuses the existing flat path (no orphaned derivative)',
    () async {
      ...
      final beforePath =
          (await db.select(db.pages).get()).single.flatRelativePath;
      ...
      expect(
        File('${base.path}/$afterPath').existsSync(),
        isTrue,
  ...
  test(
    'replacePage after reorder does not collide with another page\'s flat',
    () async {
      final r = repo();
      ...
      final page2RowBefore = pagesAfterAdd.firstWhere(
        (p) => p.relativeImagePath.endsWith('page_2.jpg'),
      );
      ...
      final pageA = allPages.firstWhere(
        (p) => p.relativeImagePath.endsWith('page_1.jpg'),
      );
      final pageB = allPages.firstWhere(
        (p) => p.relativeImagePath.endsWith('page_2.jpg'),
      );
      ...
      expect(
        pageA.flatRelativePath!.endsWith('page_1_flat.jpg'),
        isTrue,
      ...
      expect(
        pageB.flatRelativePath,
        equals(page2FlatBefore),
      ...
      expect(
        pageA.flatRelativePath,
        isNot(equals(pageB.flatRelativePath)),
        reason: 'flat paths must differ — collision means silent corruption',
      );

      // Both flat files must exist on disk.
      expect(
        File('${base.path}/${pageA.flatRelativePath}').existsSync(),
        isTrue,
        reason: 'page A flat file must exist on disk',
      );
      expect(
        File('${base.path}/${pageB.flatRelativePath}').existsSync(),
        isTrue,
