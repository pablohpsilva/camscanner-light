  ...
  group('AutoEnhancer', () {
    test('flattens a shadow gradient: shadowed background becomes near-white, '
      ...
      final bgSamples = [
        ...
        90,
        117,
      ].map((x) => out.getPixel(x, 5).luminance.toDouble()).toList();
      for (final s in bgSamples) {
        expect(
          s,
          greaterThan(220),
          reason:
              'every background sample (incl. the shadowed left) must be near-white',
        );
      }
      ...
      expect(
        out.getPixel(15, 20).luminance,
        lessThan(120),
        reason: 'shadowed-side text must remain dark',
      );
      expect(
        out.getPixel(105, 20).luminance,
        lessThan(120),
    ...
    test('image smaller than the background proxy does not crash', () async {
      ...
      expect(
        img.decodeImage(output),
        isNotNull,
        reason:
            'tiny frames skip the downscale and must still produce valid JPEG',
      );
    });

    test(
      'stretches contrast: max channel value reaches near-255 after enhancement',
      () async {
    ...
    test(
      'preserves color: output is not grayscale (R channel differs from G)',
      () async {
        ...
        expect(
          ...
          isTrue,
          reason:
              'AutoEnhancer must not convert image to grayscale (flat-field preserves hue)',
        );
      },
    );

    test(
      'returns bytes unchanged when decoding fails (corrupt data)',
      () async {
    ...
    test(
      'bakes EXIF orientation: landscape_exif6 (200x100, orient=6) becomes portrait (100x200)',
      () async {
    ...
    test(
      'uniform image (all same color) returns valid JPEG without crash',
      () async {
        final src = img.Image(width: 4, height: 4);
        for (final px in src) {
          px.r = 128;
        ...
        expect(
          ...
          isNotNull,
          reason:
              'Uniform image must not crash; degenerate auto-levels should be a no-op',
        );
      },
    );
