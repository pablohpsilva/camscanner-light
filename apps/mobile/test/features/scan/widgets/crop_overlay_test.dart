...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      const inset = CropCorners(
        topLeft: Offset(0.3, 0.3),
        topRight: Offset(0.7, 0.3),
        bottomRight: Offset(0.7, 0.7),
      ...
      expect(
        ...
        isNotNull,
        reason:
            'a drag starting on the touch target but off the visible dot '
  ...
  testWidgets('handles sit at the fitted-rect corners for full frame', (
    tester,
  ) async {
    await pump(tester);
    expect(
      tester.getCenter(find.byKey(const Key('crop-handle-tl'))),
      offsetMoreOrLessEquals(
        tester.getTopLeft(find.byKey(const Key('crop-overlay'))),
        epsilon: 1.0,
      ),
    );
  });

  testWidgets('dragging top-left emits a clamped normalized corner', (
    tester,
  ) async {
    CropCorners? out;
    await tester.pumpWidget(
  ...
  testWidgets('disabled overlay ignores drags', (tester) async {
    ...
    expect(
      out,
      isNull,
    ); // disabled => onCornersChanged never fired during the drag
  });
  ...
  group('8 handles', () {
    ...
    Widget harness(CropCorners corners) => MaterialApp(
      home: Scaffold(
        body: SizedBox(
          width: 300,
          height:
              400, // extra height so bl handle isn't on the SizedBox boundary
          child: CropOverlay(
    ...
    testWidgets('dragging the top midpoint sets topMidDev, leaves corners', (
      tester,
    ) async {
      emitted = null;
      await tester.pumpWidget(harness(straight));
      await tester.drag(
        find.byKey(const Key('crop-handle-top')),
        const Offset(0, 30),
      );
      await tester.pump();
    ...
    testWidgets('dragging a corner preserves an existing bend', (tester) async {
      ...
      await tester.drag(
        find.byKey(const Key('crop-handle-bl')),
        const Offset(15, -15),
      );
      await tester.pump();
      expect(emitted, isNotNull);
  ...
  group('cropQuadPath (curved boundary)', () {
    ...
    const straight = CropCorners(
      topLeft: Offset(0, 0.3),
      topRight: Offset(1, 0.3),
      bottomRight: Offset(1, 1),
      bottomLeft: Offset(0, 1),
    );
